...
def get_charges(real_system):
    forces = { force.__class__.__name__ : force for force in real_system.getForces() }
    nb = forces['NonbondedForce']
    charges = [nb.getParticleParameters(i)[0] for i in range(real_system.getNumParticles())]
    return charges
...
for iteration in tqdm(range(n_iter)):
    ...
    # now get the charges
old_charges = get_charges(htf[f'{phase}']._old_system)
new_charges = get_charges(htf[f'{phase}']._new_system)
np.save(f'old_charges_{phase}', np.asarray(old_charges))
np.save(f'new_charges_{phase}', np.asarray(new_charges))
